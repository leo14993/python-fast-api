...
from src.domain.repository.pessoaFisicaRepository import get_pessoas, get_pessoa_by_cpf, create_pessoa, edit_pessoa, \
    delete_pessoa, cpf_exists, insert_endereco_pessoa
...
class PessoaService():
    def __init__(self, db):
        ...
        self.check_cpf_update = lambda cpf: raise_(
            HTTPException(
                status_code=404,
                detail="Erro! O CPF informado não existe na base")
    ...
    def get(self, pessoa):
        ...
        if pessoa_exist:
            ...
            raise HTTPException(status_code=404, detail="Sem informações para este CPF")


    def create(self, pessoa):
        pessoa_exist = self.check_cpf_create(pessoa.cpf)
        if pessoa_exist:
    ...
    def update(self, pessoa):
        ...
        if evento_exist:
            ...
            raise HTTPException(status_code=404, detail="CPF não cadastrado na base")
    ...
    def create_endereco_pessoa(self, pessoa: PessoaFisicaEndereco):
        ...
        if get_pessoa:
            ...
            return insert_endereco_pessoa(db=self.db, pessoa_id=pessoa_id, endereco_id=endereco_id)
